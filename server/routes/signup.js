...
router.post('/', async (req, res) => {
    ...
    if (!name.match(nameCheck)) {
    ...
    } else if (!emailAddress.match(emailCheck)) {
        ...
        bcrypt.hash(req.body.password, 10, async function (err, hash) {
            if (err) {
                ...
                const post = await User.create({ name, emailAddress, password: hash });
                res.status(200).json({ message: 'Creating User was successful' });
            }
        });
    }
});
